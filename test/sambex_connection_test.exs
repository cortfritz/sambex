defmodule SambexConnectionTest do
  ...
  setup do
    ...
    unless Process.whereis(Sambex.DynamicConnectionSupervisor) do
      start_supervised!({DynamicSupervisor, name: Sambex.DynamicConnectionSupervisor, strategy: :one_for_one})
    end
    :ok
  end
  ...
  describe "supervised connections" do
    ...
    test "can start supervised named connection" do
      {:ok, pid} = Sambex.ConnectionSupervisor.start_connection(
        url: "smb://test.example.com/share",
        username: "testuser",
        password: "testpass", 
  ...
  describe "documentation" do
    ...
    test "Sambex.ConnectionSupervisor has proper moduledoc" do
      {:docs_v1, _, :elixir, _, %{"en" => module_doc}, _, _} = Code.fetch_docs(Sambex.ConnectionSupervisor)
